<div class="bg-white rounded-2xl shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300 border border-gray-100 group overflow-hidden h-full flex flex-col transform hover:-translate-y-1">
    <div class="relative">
        <a href="{{ route('products.show', $product->slug) }}" class="block overflow-hidden aspect-[4/5] bg-gray-50">
            @if($product->primaryImage)
                <img src="{{ $product->primaryImage->image_path }}" alt="{{ $product->name }}" class="h-full w-full object-cover object-center group-hover:scale-105 transition duration-700 ease-in-out">
            @else
        ...
        <div class="absolute bottom-0 left-0 right-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
             <a href="{{ route('products.show', $product->slug) }}" class="block w-full bg-white/90 backdrop-blur text-gray-900 font-bold py-3 rounded-xl shadow-lg border border-gray-100 text-center hover:bg-blue-600 hover:text-white transition-colors text-sm">
                View Details
            </a>
        </div>
    </div>
    ...
    <div class="p-5 flex-1 flex flex-col">
        <div class="mb-2">
            <span class="text-xs font-medium text-blue-600 uppercase tracking-wide bg-blue-50 px-2 py-1 rounded-md">{{ $product->brand ? $product->brand->name : config('app.name') }}</span>
        </div>
        ...
        <div class="mt-auto pt-4 flex items-end justify-between border-t border-gray-50">
            ...
            <div class="flex items-center gap-1">
                ...
                <span class="text-xs font-bold text-gray-700">{{ $product->average_rating }}</span>
            </div>
        </div>
    </div>
</div>
